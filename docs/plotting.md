...
└── plots/
    ...
    │   ├── data_set_overview/
    │   │   ├── raw_filter_<feature>.png       (per pre-filter raw stat)
    │   │   └── quality_score_<feature>.png    (per quality component)
    │   └── bad_examples/
...
- `data_set_overview/` — each image has a distribution histogram on the left (rejected stacked by reason, accepted in teal, selected frames marked by black dashed vertical lines) and the feature value over the frame sequence on the right, points coloured by matplotlib's `coolwarm` (cold/blue = low value, warm/red = high value), selected frames highlighted in gold. The histogram x-axis is fixed to `[-0.05, 1.05]` for features whose values are bounded to `[0, 1]`.
- `bad_examples/` — the five worst accepted frames per feature, each with the mask overlaid (artifact pixels highlighted for `vincent_artifact_fraction`, the boundary band for `vincent_boundary_blur_variance`, border pixels for the border/edge features). Every thumbnail is framed by a `coolwarm` border proportional to its feature value and labelled with the frame id and value.
