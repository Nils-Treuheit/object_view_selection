...
on the raw stat. **Not** part of the default `filter_order`; used as a
diagnostic and, optionally, as a rejection layer.
...
1. **Raw stat** (per observation, `evaluate`): variance of the Laplacian
   restricted to the boundary band (`compute_boundary_blur_variance`):
...
2. **Optional hard floor:** if `hard_min_variance > 0` and the stat is below
   it, the observation is rejected outright with reason `motion_blur`.

3. **Population pass** (`fit_weights`): same robust median/MAD one-sided
   half-Gaussian falloff as `VincentsAreaFilter`, penalising the "bad" (low =
   blurred) side:
   ...
   small (`0.3` robust-MADs) to discriminate.
...
Without a hard floor the filter returns `(1.0, True, "")`. Two rejection paths
exist:

- **Absolute:** `stat < hard_min_variance` → `(0.0, False, "motion_blur")`.
  ...
  static bottle set — that the soft weight would otherwise merely down-rank.
- **Population (optional):** `threshold_min` / `outlier_z` on the fitted
  weight via `reject_soft_variants`:
