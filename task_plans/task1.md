...
5. **Webapp defaults to 2D** — NOT DONE. Template initialises
   `currentDims = "3d"` with `dims_3d` active; `webapp.py` defaults `dims="3d"`.
   Switch defaults to `"2d"` so the first render is the clean 2D scatter.
6. **Docs** — `docs/explorer.md` updated (pure-MDS behaviour, white theme,
   2D/3D switch). Update it again after the 2D default change.
